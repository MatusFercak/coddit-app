...
<div class="row row-cols-1 row-cols-lg-3 align-items-stretch g-4 py-5">
    ...
    <div class="col hover-zoom" data-toggle="popover" on:click={onClick(c.$id)} >
        <div class="card card-cover h-100 overflow-hidden text-white bg-dark rounded-4 shadow-lg" style={`background-image: url('${getImg(c.imgId)}');`}>
...
{#if $user !== null}
    <div class="mt-5 d-flex justify-content-center">
        <button type="button" class="btn btn-primary btn-lg btn-block" on:click={onCreate}>Add subcoddit.</button>
    </div>
{/if} 
